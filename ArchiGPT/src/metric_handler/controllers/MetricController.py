...
def getProjectJson():
    try:
            ...
            for service in containerData['services']:
                ...
                if service['type'] == "backend" or service['type'] == "frontend":
                    ...
                    if service['type'] == "backend":
                        ...
                        temp_endpoints = service['ServiceEndpointGenerator']

                        archiProject["containers"][indexContainer]["services"].append({
                            "name": service['name'],
                            "type": service['type'],
                            "endpoints": endpointsFormatter(temp_endpoints)
                        })        
                
                    if service['type'] == "frontend" :

                        temp_pages = service['ServicePageGenerator']

                        archiProject["containers"][indexContainer]["services"].append({
                            "name": service['name'],
                            "type": service['type'],
                            "pages": pagesFormatter(temp_pages)
                        })
        ...
        print("Project Json : ", archiProject)
